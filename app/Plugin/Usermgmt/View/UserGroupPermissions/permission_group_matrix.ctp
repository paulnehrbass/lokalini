...
<div class="um-panel">
	<div class="um-panel-header">
		...
		<span class="um-panel-title">
			<?php echo $this->Html->link(__('View Standard', true), '/permissions');?>
		</span>
	...
	<div class="um-panel-content">
		<div style='padding:10px'><?php echo $this->Html->image(SITE_URL.'usermgmt/img/approve.png', array("alt" => __('Yes')));?> = The group has permission of controller's action<br/><?php echo $this->Html->image(SITE_URL.'usermgmt/img/dis-approve2.png', array("alt" => __('Yes')));?> = The group has not permission of controller's action<br/></div>
		<?php   $tDisplay='none';
		...
		<div style='padding:10px; display:<?php echo $tDisplay; ?>' id='perOptionsDetails'>
			...
			<div style='float:left'>
				<table class="table table-striped table-bordered table-hover" style='width:auto'>
					<thead>
						<tr>
							<th><?php echo $this->Form->input('sel_cont_all' ,array('type'=>'checkbox', 'div' => false, 'label' => false, 'checked' => false, 'hiddenField' => false, 'class'=>'contcheckall')); ?></th>
			...
			<?php       if (!empty($allControllers)) {
							foreach ($allControllers as $key=>$cont) {
								...
								echo "<tr>";
								echo "<td>";
								echo $this->Form->input('controller.'.$key.'.contcheck', array('type'=>'checkbox', 'div' => false, 'label' => false, 'checked' => $checked, 'hiddenField' => false, 'class'=>'contcheck'));
								echo "</td>";
								echo "<td>".$cont."</td>";
								echo "</tr>";
							}
			...
			<div style='float:left'>
				<table class="table table-striped table-bordered table-hover" style='width:auto'>
					<thead>
						<tr>
							<th><?php echo $this->Form->input('sel_grp_all', array('type'=>'checkbox', 'div' => false, 'label' => false, 'checked' => false, 'hiddenField' => false, 'class'=>'grpcheckall')); ?></th>
			...
			<?php       if (!empty($user_groups)) {
							foreach ($user_groups as $key=>$group) {
								...
								echo "<tr>";
								echo "<td>";
								echo $this->Form->input('group.'.$group['id'].'.grpcheck', array('type'=>'checkbox', 'div' => false, 'label' => false, 'checked' => $checked, 'hiddenField' => false, 'class'=>'grpcheck'));
								echo "</td>";
	...
	<?php           foreach ($controllers as $cont=>$actions) {
						...
						foreach ($actions as $key=>$action) {
							...
							foreach ($sel_user_groups as $group) {
								echo "<td style='text-align:center;padding:5px'><div style='width:55px;height:20px'>";
								if (isset($controllerActions[$cont][$action][$group['alias_name']]) && $controllerActions[$cont][$action][$group['alias_name']]==1) {
									echo $this->Js->link($this->Html->image(SITE_URL.'usermgmt/img/approve.png', array('alt' => __('Yes'))), array('action' => 'changePermission', $cont, $action, $group['id']), array('escape' => false, 'title' => $group['name'].' (Click to change permission)', 'before'=>"var targetId = event.currentTarget.id; $('#'+targetId).html('".$loadingImg."');", 'success'=>"var targetId = event.currentTarget.id; if(data) { $('#'+targetId).html(data); }"));
								} else {
									echo $this->Js->link($this->Html->image(SITE_URL.'usermgmt/img/dis-approve2.png', array('alt' => __('No'))), array('action' => 'changePermission', $cont, $action, $group['id']), array('escape' => false, 'title' => $group['name'].' (Click to change permission)', 'before'=>"var targetId = event.currentTarget.id; $('#'+targetId).html('".$loadingImg."');", 'success'=>"var targetId = event.currentTarget.id; if(data) { $('#'+targetId).html(data); }"));
								}
								echo "</div></td>";
							}
							echo "</tr>";
